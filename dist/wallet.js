...
const directories = [homedir + '/.xte-cli-wallet', homedir + '/.xte-cli-wallet/logs', homedir + '/.xte-cli-wallet/wallets'];
...
let walletLogStream = fs_1.default.createWriteStream(logDirectory + '/xte-wallet.log', {
    flags: 'a'
});
// set daemon for TRAAITT network using RPC daemon (host, port, isCacheApi, ssl)
// Using standard RPC daemon on port 14485 for full compatibility
const daemon = new WB.Daemon('main.trrxitte.com', 14485, false, false);
// Configure wallet backend for FAST SYNC (optimized for speed)
const customConfig = new WB.Config();
customConfig.blocksPerTick = 100; // Process 100 blocks per tick for maximum speed
customConfig.syncThreadInterval = 10; // Process as fast as possible (10ms interval)
customConfig.requestTimeout = 60000; // 60 second timeout for large batch requests
customConfig.scanCoinbaseTransactions = false; // Skip coinbase txs for faster sync
customConfig.daemonUpdateInterval = 30000; // Only update daemon info every 30s (reduce overhead)
...
// Apply custom config to daemon
daemon.updateConfig(customConfig);
// initialize daemon connection
let daemonInitialized = false;
daemon.init().then(() => {
    daemonInitialized = true;
    logFile('Daemon initialized successfully');
}).catch((error) => {
    logFile('Daemon initialization error: ' + error);
});
...
function drawWalletWindow(fileName, password) {
    ...
    if (error) {
        logFile(error);
        // Check for common incompatibility errors
        if (error.toString().includes('different network') ||
            error.toString().includes('Invalid wallet') ||
            error.toString().includes('version')) {
            return new Error('Wallet file incompatible with TRAAITT network. Please create a new wallet or import TRAAITT keys.');
        }
        return error;
    }
    // delete log file if currently present
    if (fs_1.default.existsSync(logDirectory + `${fileName}.log`)) {
        fs_1.default.unlinkSync(logDirectory + `${fileName}.log`);
    }
    ;
    // configure logging
    wallet.setLogLevel(WB.LogLevel.DEBUG);
    wallet.setLoggerCallback((prettyMessage, message, level, categories) => {
        let logStream = fs_1.default.createWriteStream(logDirectory + '/xte-wallet.log', {
            flags: 'a'
        });
        logStream.write(prettyMessage + '\n');
    });
    // Add sync progress handler with completion detection (minimal logging for speed)
    wallet.on('sync', (walletHeight, networkHeight) => {
        if (Math.random() < 0.01) { // Log only 1% of sync events for fast sync
            logFile(`Fast syncing: ${walletHeight} / ${networkHeight}`);
        }
    });
    // Add height change handler for fast sync (reduced logging)
    let lastHeightLog = 0;
    let lastHeightTime = Date.now();
    let stuckWarningShown = false;
    wallet.on('heightchange', (walletHeight, localHeight, networkHeight) => {
        const now = Date.now();
        const timeSinceLastHeight = now - lastHeightTime;
        // Log every 1000 blocks for fast sync (less overhead)
        if (walletHeight % 1000 === 0 || walletHeight - lastHeightLog > 1000) {
            logFile(`Fast sync progress: ${walletHeight}/${networkHeight} (${((walletHeight / networkHeight) * 100).toFixed(1)}%)`);
            lastHeightLog = walletHeight;
            stuckWarningShown = false;
        }
        // Warn if stuck on same height for more than 60 seconds (fast sync can be slow on complex blocks)
        if (timeSinceLastHeight > 60000 && !stuckWarningShown) {
            logFile(`WARNING: Sync stalled at height ${walletHeight} for ${timeSinceLastHeight / 1000}s`);
    ...
    wallet.on('disconnect', (error) => {
        logFile(`Daemon disconnected: ${error}`);
    });
    // Add reconnect handler
    wallet.on('connect', () => {
        logFile('Daemon reconnected');
    });
    // Add dead node handler
    wallet.on('deadnode', () => {
        logFile('WARNING: Daemon appears to be dead or unresponsive');
    });
    // start the wallet
    wallet.start();
    ...
    // refresh the sync information every 2 seconds for fast sync (less UI overhead)
    refreshSync(syncStatus, wallet, progress);
    setInterval(refreshSync.bind(null, syncStatus, wallet, progress), 2000);
    // Periodically refresh balance to ensure it stays synced (every 2 seconds for instant updates)
    setInterval(() => {
        try {
            const currentBalance = wallet.getBalance();
            walletBalance.setContent(`{|}{bold}${WB.prettyPrintAmount(currentBalance[0])}{/}\n` +
                `{|}{bold}{red-fg}${WB.prettyPrintAmount(currentBalance[1])}{/}\n` +
                `{|}${WB.prettyPrintAmount(currentBalance[1] + currentBalance[0])}`);
            // Update available balance on transfer form as well
            availableBalanceText.setContent(`{bold}Available:{/}\n${WB.prettyPrintAmount(currentBalance[0])}`);
            screen.render();
        }
        catch (error) {
            logFile(`Error in periodic balance refresh: ${error}`);
        }
    }, 2000);
    // Periodically update daemon info to keep connection fresh (every 30 seconds)
    setInterval(() => {
        daemon.updateDaemonInfo().catch((error) => {
            logFile(`Daemon update error: ${error}`);
        });
    }, 30000);
    ///////////////////////////////////////////////////////////////////
    //  TRANSFER WINDOW
    ...
    transferForm.on('submit', function (data) {
        return __awaiter(this, void 0, void 0, function* () {
            let paymentID;
            if (data.paymentid === '') {
                paymentID = undefined;
            }
            const [hash, error] = yield wallet.sendTransactionBasic(data.address, atomicUnits(parseInt(data.amount)), paymentID);
            if (error) {
                logFile(error);
                yield notifyUser(errorText, error.toString(), 5000);
                transferForm.reset();
            }
            else {
                logFile(`Transaction sent successfully. Hash: ${hash}`);
                transferWindow.hide();
                walletWindow.show();
                walletWindow.setFront();
                walletWindow.focus();
                // Immediately update balance (first pass)
                try {
                    const immediateBalance = wallet.getBalance();
                    walletBalance.setContent(`{|}{bold}${WB.prettyPrintAmount(immediateBalance[0])}{/}\n` +
                        `{|}{bold}{red-fg}${WB.prettyPrintAmount(immediateBalance[1])}{/}\n` +
                        `{|}${WB.prettyPrintAmount(immediateBalance[1] + immediateBalance[0])}`);
                    availableBalanceText.setContent(`{bold}Available:{/}\n${WB.prettyPrintAmount(immediateBalance[0])}`);
                    screen.render();
                }
                catch (updateError) {
                    logFile(`Error in immediate balance update: ${updateError}`);
                }
                yield notifyUser(notificationText, `{white-fg}{bold}Sent succesfully!{/bold}\nHash: ${hash.substring(0, 16)}...`, 3000);
                // Delayed refresh to ensure wallet backend has processed the transaction
                setTimeout(() => {
                    try {
                        const updatedBalance = wallet.getBalance();
                        walletBalance.setContent(`{|}{bold}${WB.prettyPrintAmount(updatedBalance[0])}{/}\n` +
                            `{|}{bold}{red-fg}${WB.prettyPrintAmount(updatedBalance[1])}{/}\n` +
                            `{|}${WB.prettyPrintAmount(updatedBalance[1] + updatedBalance[0])}`);
                        // Update transaction table
                        const updatedTxList = wallet.getTransactions();
                        const updatedTxArray = getRecentTransactions(updatedTxList);
                        transactionTable.setData({
                            headers: ['Time', 'Amount'],
                            data: updatedTxArray
                        });
                        // Update available balance on transfer form
                        availableBalanceText.setContent(`{bold}Available:{/}\n${WB.prettyPrintAmount(updatedBalance[0])}`);
                        screen.render();
                        logFile(`Balance updated after send: Available=${WB.prettyPrintAmount(updatedBalance[0])}, Locked=${WB.prettyPrintAmount(updatedBalance[1])}`);
                    }
                    catch (delayedUpdateError) {
                        logFile(`Error in delayed balance update: ${delayedUpdateError}`);
                    }
                }, 100); // 100ms delay to allow wallet backend to process
                // Additional refresh after 500ms for locked balance updates
                setTimeout(() => {
                    try {
                        const finalBalance = wallet.getBalance();
                        walletBalance.setContent(`{|}{bold}${WB.prettyPrintAmount(finalBalance[0])}{/}\n` +
                            `{|}{bold}{red-fg}${WB.prettyPrintAmount(finalBalance[1])}{/}\n` +
                            `{|}${WB.prettyPrintAmount(finalBalance[1] + finalBalance[0])}`);
                        availableBalanceText.setContent(`{bold}Available:{/}\n${WB.prettyPrintAmount(finalBalance[0])}`);
                        screen.render();
                    }
                    catch (finalUpdateError) {
                        logFile(`Error in final balance update: ${finalUpdateError}`);
                    }
                }, 500);
            }
        });
    ...
    // on transaction - completely skip processing during sync to prevent slowdown
    let lastTxUpdate = 0;
    let transactionsPending = false;
    wallet.on('transaction', function (transaction) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // Check if actively syncing (more than 100 blocks behind)
                const [walletHeight, , networkHeight] = wallet.getSyncStatus();
                const isSyncing = walletHeight < networkHeight - 100;
                // During sync, just mark that we have pending transactions and skip all processing
                if (isSyncing) {
                    transactionsPending = true;
                    // Log every 100th transaction to avoid log spam
                    if (Math.random() < 0.01) {
                        logFile(`Syncing transactions... (${walletHeight}/${networkHeight})`);
                    }
                    return; // Skip all processing during sync
                }
                // Only process when fully synced or nearly synced
                const now = Date.now();
                // Throttle updates even when synced (max every 2 seconds)
                if ((now - lastTxUpdate) < 2000) {
                    return;
                }
                lastTxUpdate = now;
                ...
                const txAmount = transaction.totalAmount();
                const txType = txAmount >= 0 ? 'received' : 'sent';
                const txConfirmed = transaction.unlockTime === 0 || transaction.blockHeight > 0;
                logFile(`Transaction ${txType}: ${txHash} - Amount: ${WB.prettyPrintAmount(Math.abs(txAmount))} - Confirmed: ${txConfirmed}`);
                // Show notification for new transactions
                const notificationMsg = txType === 'received'
                    ? `Received ${WB.prettyPrintAmount(Math.abs(txAmount))} XTE!`
                    : `Sent ${WB.prettyPrintAmount(Math.abs(txAmount))} XTE - Confirming...`;
                notifyUser(notificationText, notificationMsg, 3000);
                // Update balance
                ...
                // Update available balance on transfer form
                availableBalanceText.setContent(`{bold}Available:{/}\n${WB.prettyPrintAmount(updateBalance[0])}`);
                screen.render();
            }
            catch (error) {
                logFile(`Error processing transaction: ${error}`);
            }
        });
    });
    // When sync completes, refresh everything once
    wallet.on('sync', (walletHeight, networkHeight) => {
        if (Math.abs(walletHeight - networkHeight) < 10 && transactionsPending) {
            logFile('Sync complete, refreshing transaction data...');
            setTimeout(() => {
                try {
                    let finalBalance = wallet.getBalance();
                    walletBalance.setContent(`{|}{bold}${WB.prettyPrintAmount(finalBalance[0])}{/}\n` +
                        `{|}{bold}{red-fg}${WB.prettyPrintAmount(finalBalance[1])}{/}\n` +
                        `{|}${WB.prettyPrintAmount(finalBalance[1] + finalBalance[0])}`);
                    let finalTransactionList = wallet.getTransactions();
                    const finalTxArray = getRecentTransactions(finalTransactionList);
                    transactionTable.setData({
                        headers: ['Time', 'Amount'],
                        data: finalTxArray
                    });
                    ...
                    screen.render();
                    transactionsPending = false;
                    notifyUser(notificationText, 'Sync complete!', 3000);
                }
                catch (error) {
                    logFile(`Error in post-sync refresh: ${error}`);
                }
            }, 1000);
        }
    });
...
function createWallet(fileName, password) {
    if (fs_1.default.existsSync(walletDirectory + `/${fileName}.wallet`)) {
...
function refreshSync(syncStatus, wallet, progress) {
    try {
        ...
        // Add syncing status indicator
        const isSyncing = walletHeight < networkHeight - 10;
        const syncLabel = isSyncing ? '{yellow-fg}Syncing...{/}' : '{green-fg}Synced{/}';
        syncStatus.setContent(` {bold}Status:{/} ${syncLabel} {|}{red-fg}${walletHeight}{/red-fg} of{/bold} ${networkHeight} \n` +
            `{|}${percentSync.toFixed(2)}% `);
        progress.setProgress(percentSync);
        screen.render();
    }
    catch (error) {
        logFile(`Error in refreshSync: ${error}`);
    }
}
;
...
function convertTimestamp(timestamp) {
    ...
    hh = ('0' + d.getHours()).slice(-2), min = ('0' + d.getMinutes()).slice(-2), // Add leading 0.
    time;
    // ie: 2013-02-18, 16:35
