...
// imports
import WB = require('@trrxitte/xte-wallet-backend');
...
let walletLogStream = fs.createWriteStream(logDirectory + '/xte-wallet.log', {
    flags: 'a'
});
...
function drawWalletWindow(fileName, password) {
    ...
    wallet.setLoggerCallback((prettyMessage, message, level, categories) => {
        let logStream = fs.createWriteStream(logDirectory + '/xte-wallet.log', {
            flags: 'a'
        });
    ...
    setInterval(() => {
        try {
            ...
            walletBalance.setContent(
                `{|}{bold}${WB.prettyPrintAmount(currentBalance[0])}{/}\n` +
    ...
    transferForm.on('submit', async function(data) {
        ...
        if (error) {
            ...
            try {
                ...
                walletBalance.setContent(
                    `{|}{bold}${WB.prettyPrintAmount(immediateBalance[0])}{/}\n` +
            ...
            await notifyUser(notificationText, `{white-fg}{bold}Sent succesfully!{/bold}\nHash: ${hash.substring(0, 16)}...`, 3000);
            ...
            setTimeout(() => {
                try {
                    ...
                    walletBalance.setContent(
                        `{|}{bold}${WB.prettyPrintAmount(updatedBalance[0])}{/}\n` +
            ...
            setTimeout(() => {
                try {
                    ...
                    walletBalance.setContent(
                        `{|}{bold}${WB.prettyPrintAmount(finalBalance[0])}{/}\n` +
    ...
    wallet.on('transaction', async function(transaction) {
        try {
...
function importWallet(fileName: string, password: string, seed: string, startHeight: number) {
    if (fs.existsSync(walletDirectory + `/${fileName}.wallet`)) {
        return `${fileName}.wallet already exists, will not overwrite.`;
    };
...
function refreshSync(syncStatus, wallet, progress) {
    try {
        ...
        syncStatus.setContent(
            ` {bold}Status:{/} ${syncLabel} {|}{red-fg}${walletHeight}{/red-fg} of{/bold} ${networkHeight} \n` +
